...
output_directory = os.path.join(os.getcwd(), 'Map3_Including_Not_T_Length_File')
...
def process_replicate(file_path):
    ...
    # Filter rows
    #UPDATE if you want diff parameters, for example if you want to onlycapture a specific length tile but right now saves any length tile 
    #if the tile length is wrong then it automatically means the tile is not in the design file
    map2 = df[df['A Qual'] == 1]
    ...
    # map2_path = os.path.join(output_directory, f'{base_name}_Not_In__T_Length_Map2.csv')
    ...
    # Create Map3
    map3 = map2.drop(columns=['Reads', 'Designed', 'A Qual', 'A Len'])
    ...
    # Save Map3 including Tiles with Incorrect Tile Length 
    #map3_path = os.path.join(output_directory, f'{base_name}_Map3_with_NOT_in_T_Length.csv') #UPDATE saves csv with all rows of normal map3 + all rows where there is a correct bc1 length (A Qual == 1) but wrong tile length
    #map3.to_csv(map3_path, index=False)

    #save Map3 of Only Tiles with Incorrect Tile Length 
    map3_filteredx = map3[map3['T Qual'] == 0]
    #map3_pathx = os.path.join(output_directory, f'{base_name}_Map3_ONLY_NOT_in_T_Length.csv') #UPDATE can create a csv file with all rows where tile length is incorrect, even repeat rows
    ...
    map3_ge5_unique_fpd = os.path.join(output_directory, f'{base_name}_Map3_ONLY_NOT_in_T_Length_LUT.csv') #UPDATE this saves a csv look up table of only unique tile + bc1 combintation where T Qual == 0 
    ...
    # Count unique values in the Tiles column where T Qual == 0 or 1
    tcov_designed_0 = map3.loc[map3['T Qual'] == 0, 'Tiles'].nunique()
    tcov_designed_1 = map3.loc[map3['T Qual'] == 1, 'Tiles'].nunique()

    #Count ADBC that appear in both the designed and not designed Tiles
    tiles_designed_0s = set(map3.loc[map3['T Qual'] == 0, 'AD BCs'].unique())
    tiles_designed_1s = set(map3.loc[map3['T Qual'] == 1, 'AD BCs'].unique())
    ...
    #Count Cat that appear in both theT Qual and not v Tiles
    tcov_designed_0c = map3.loc[map3['T Qual'] == 0, 'Cat'].nunique()
    tcov_designed_1c = map3.loc[map3['T Qual'] == 1, 'Cat'].nunique()
    ...
    summary_dict['Category'].append(f'{base_name} Not in T Qual == 1 File Unique Tiles Count')
    summary_dict['Read Count'].append(tcov_designed_0)
    summary_dict['Category'].append(f'{base_name} In T Qual ==1 File Unique Tiles Count')
    ...
    summary_dict['Category'].append(f'{base_name} Not in T Qual == 1 File Unique Tile + BC Count')
    summary_dict['Read Count'].append(tcov_designed_0c)
    summary_dict['Category'].append(f'{base_name} In T Qual == 1 File Unique Tile + BC Count')
    ...
    summary_dict['Category'].append(f'{base_name} Not in T Qual == 1 File Unique ADBCs Count')
    summary_dict['Read Count'].append(len(tiles_designed_0s))
    summary_dict['Category'].append(f'{base_name} In T Qual == 1 File Unique ADBCs Count')
    summary_dict['Read Count'].append(len(tiles_designed_1s))
    summary_dict['Category'].append(f'{base_name} ADBCs Overlap in T Qual == 1 and Not in T Qual == 1 Count')
...
summary_path = os.path.join(output_directory, 'Summary_Not_In_T_Len_File.csv')
